...
================================================================================
YOU MUST check the input size BEFORE writing any other code!
...
================================================================================
DECISION TREE: Follow the path that matches your input size
================================================================================
You have TWO approaches available. The size determines which one to use.
Follow the instructions for YOUR PATH exactly as written.
Do NOT improvise or write custom code!

--------------------------------------------------------------------------------
PATH A: Small Codebase (< 50,000 characters)
--------------------------------------------------------------------------------
If input_size < 50000, use THIS approach:

if input_size < 50000:
    # ⚠ YOU ARE ON PATH A - Follow these steps EXACTLY:
    #
    # DIRECT APPROACH: Pass entire codebase to sub_llm for semantic matching
    # This approach avoids fragmentation errors by giving sub_llm full context
    #
    # Step 1: Ask sub_llm to identify the function name from ENTIRE codebase
    func_name = sub_llm(
    ...
    # Step 2: Search for the identified function in the full code
    res = search(input_0, "def " + func_name + "(")
    if not res: res = search(input_0, "    def " + func_name + "(")  # Try indented (class method)
    if not res: res = search(input_0, "def " + func_name)  # Try without parentheses
    if not res: res = search(input_0, func_name + "(")  # Try function call pattern
    if not res: res = search(input_0, func_name)  # Try name alone

    # Step 3: Return the function with surrounding context
    if res:
        match, before, after = res[0]
        FINAL(func_name + "||" + before[-800:] + match + after[:5000])
    else:
        # Fallback: Try to find by string position
    ...
    # ⚠ IMPORTANT: If you are on PATH A (input_size < 50000), you are DONE!
    # Do NOT execute the PATH B code below - it is for large codebases only.
    # The PATH A code above already called FINAL(), so execution stops here.

--------------------------------------------------------------------------------
PATH B: Large Codebase (>= 50,000 characters)
--------------------------------------------------------------------------------
If input_size >= 50000, use THIS approach:

# EXTRACTION APPROACH: Sample function signatures and use sub_llm to pick
# This approach is necessary when the entire codebase won't fit in sub_llm context

# Step 1: Extract function signatures from a preview window
preview_size = 8000
preview = input_0[:preview_size]
func_name = None
res = None

# Try up to 2 iterations: small window first, then large window if needed
for attempt in range(2):
    # Extract all function names from preview
    ...
    if func_names:
        ...
        # Extract function SIGNATURES + DOCSTRINGS for context
        func_infos = []
        for name in unique_funcs[:25]:
            ...
            if sig_match:
                ...
                # Get next 3 lines after signature for docstring
                ...
                if lines_after:
                    ...
                    if docstring_preview:
                        func_snippet += '\n    ' + docstring_preview[:150]

                func_infos.append(func_snippet)

        # Give sub_llm the function list with signatures + docstrings
        ...
        # Validate sub_llm returned an actual function name
        if func_name not in unique_funcs:
            ...
            if best_match:
                ...
                func_name = unique_funcs[0]  # Last resort
    else:
    ...
    # Step 2: Search for the function definition
    res = search(input_0, "def " + func_name + "(")
    ...
    # If found OR already tried large window, stop
    if res or attempt == 1:
        break

    # Not found and first attempt - expand window and retry
    preview_size = 32000
    preview = input_0[:preview_size]

# Step 3: Return function with context
if res:
    match, before, after = res[0]
    FINAL(func_name + "||" + before[-800:] + match + after[:5000])
else:
...
7) OUTPUT: Return clean values only to FINAL()
   - FINAL("correct") not FINAL("Label: correct")
   - FINAL("18") not FINAL("Answer: 18")
   - FINAL("44106") not FINAL("User: 44106")
